...
original_author="2021UESTCDbExperimentGroup18"
original_name="flask_backend"
original_urlname="flask_backend"
original_description="Awesome flask_backend created by 2021UESTCDbExperimentGroup18" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_backend "${name}"
...
then
    ...
    echo "then run 'flask_backend' or 'python -m flask_backend'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_backend' or 'python -m flask_backend' or 'poetry run flask_backend'"
fi
